Shader "PostProcessing/SobelOutline"
{
    ...
    float4 SobelSample(Texture2D t, SamplerState s, float3 offset, float2 uv)
    {
    ...
    float SobelSampleDepth(Texture2D t, SamplerState s, float2 uv)
    {
        float3 o = float3((1.0 / _ScreenParams.x), (1.0 / _ScreenParams.y), 0.0) * _OutlineThickness;

        float pixelCenter = LinearEyeDepth(t.Sample(s, uv).r);
        float pixelLeft = LinearEyeDepth(t.Sample(s, uv - o.xz).r);
        float pixelRight = LinearEyeDepth(t.Sample(s, uv + o.xz).r);
        float pixelUp = LinearEyeDepth(t.Sample(s, uv + o.zy).r);
        float pixelDown = LinearEyeDepth(t.Sample(s, uv - o.zy).r);
